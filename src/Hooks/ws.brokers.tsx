...
export function useWebSocketBrokers(
  url: string = 'ws://localhost:2003/analysis',
  {
    autoConnect = true,
    autoReconnect = true,
    reconnectDelay = 3000,
  ...
  const [connected_brokers, setConnected] = useState(false);
  ...
  const connect_Brokers = useCallback(() => {
  ...
  const disconnect_Brokers = useCallback(() => {
  ...
  useEffect(() => {
    if (autoConnect) connect_Brokers();
    return () => {
      // cleanup khi unmount
      disconnect_Brokers();
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [url, autoConnect, connect_Brokers, disconnect_Brokers]);

  const readyState =
    wsRef.current?.readyState ?? WebSocket.CLOSED; // 0 CONNECTING, 1 OPEN, 2 CLOSING, 3 CLOSED

  return { brokers, connected_brokers, readyState, connect_Brokers, disconnect_Brokers };
}
