...
export function useWebSocketBrokerInfo(
  url: string = 'ws://localhost:2002/symbols-broker-info?broker=ICmarket',
  {
    autoConnect = true,
    autoReconnect = true,
    reconnectDelay = 3000,
  ...
  const [connected_brokerInfo, setConnected] = useState(false);
  const [brokerInfo, setBrokers] = useState<any>(null);
  ...
  const connect_BrokerInfo = useCallback(() => {
  ...
  const disconnect_BrokerInfo = useCallback(() => {
  ...
  useEffect(() => {
    if (autoConnect) connect_BrokerInfo();
    return () => {
      // cleanup khi unmount
      disconnect_BrokerInfo();
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [url, autoConnect, connect_BrokerInfo, disconnect_BrokerInfo]);

  const readyState =
    wsRef.current?.readyState ?? WebSocket.CLOSED; // 0 CONNECTING, 1 OPEN, 2 CLOSING, 3 CLOSED

  return { brokerInfo, connected_brokerInfo, readyState, connect_BrokerInfo, disconnect_BrokerInfo };
}
