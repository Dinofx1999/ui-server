...
export function useWebSocketSymbols(
  url: string = 'ws://116.105.227.149:2000/symbol-brokers?symbol=GBPUSD',
  {
    autoConnect = true,
    autoReconnect = true,
    reconnectDelay = 3000,
  ...
  const [connected_symbols, setConnected] = useState(false);
  const [symbols, setAnalysis] = useState<any>(null);
  ...
  const connect_symbols = useCallback(() => {
  ...
  const disconnect_symbols = useCallback(() => {
  ...
  useEffect(() => {
    if (autoConnect) connect_symbols();
    return () => {
      // cleanup khi unmount
      disconnect_symbols();
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [url, autoConnect, connect_symbols, disconnect_symbols]);

  const readyState =
    wsRef.current?.readyState ?? WebSocket.CLOSED; // 0 CONNECTING, 1 OPEN, 2 CLOSING, 3 CLOSED

  return { symbols, connected_symbols, readyState, connect_symbols, disconnect_symbols };
}
